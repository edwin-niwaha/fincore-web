...
export default function RegisterPage() {
  ...
  const handleGoogleToken = useCallback(async (accessToken: string) => {
    ...
    router.replace(user.is_email_verified ? dashboardPathForRole(user.role) : '/verify-email');
  }, [loginWithGoogle, router]);

  return (
    <AuthCard title="Create your account" subtitle="Register as a client user. Admin and staff accounts can be managed by authorized users.">
      <form className="grid gap-4" onSubmit={handleSubmit(onSubmit)}>
        <Field label="Email address" error={errors.email?.message}>
          <Input autoComplete="email" type="email" placeholder="you@example.com" {...register('email')} />
        </Field>
        <Field label="Username" error={errors.username?.message}>
          <Input autoComplete="username" placeholder="edwin" {...register('username')} />
        </Field>
        <Field label="Password" error={errors.password?.message}>
          <Input autoComplete="new-password" type="password" placeholder="At least 8 characters" {...register('password')} />
        </Field>
        <Field label="Confirm password" error={errors.password_confirm?.message}>
          <Input autoComplete="new-password" type="password" placeholder="Repeat password" {...register('password_confirm')} />
        </Field>
      ...
      <p className="mt-6 text-center text-sm text-slate-600">
        Already have an account? <Link className="font-bold text-[#127D61]" href="/login">Sign in</Link>
      </p>
    </AuthCard>
  );
}
