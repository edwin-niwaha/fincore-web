...
import type { LoanCollectionsReport, LoanProduct } from '@/types/api';
import { LoanReportMetricCard } from './loan-report-metric-card';
import { reportScopeValue } from './report-utils';

export function LoanCollectionsReportPage() {
  ...
  const loadReport = useCallback(
    () =>
      accountingApi.reports.loanCollections({
  ...
  const { data, error, isLoading, reload } =
    useApiResource<LoanCollectionsReport>(loadReport);
  ...
  if (
    ...
    return <StateView title="Loading loan collections report..." />;
  }
  ...
  if (error && !data) {
    return (
      <StateView
        title="Could not load collections report"
        description={error}
        actionLabel="Retry"
        onAction={reload}
      />
    ...
    <div className="grid gap-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <PageHeader
          ...
          description="Review repayments collected across products, branches, and staff collection methods."
        />
        <Button
          type="button"
      ...
      <Card className="overflow-hidden p-0">
        <div className="border-b border-slate-200 bg-slate-50/80 px-5 py-4">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Filter repayment collections by date range, branch, and product.
          </p>
        </div>
        <div className="grid gap-4 p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <LoanReportMetricCard
              ...
              helper="Repayment entries in the selected reporting window."
            />
            <LoanReportMetricCard
              label="Total collected"
              value={moneyPrecise(data?.totals?.amount)}
              helper="Combined value of repayments recorded."
            />
            <LoanReportMetricCard
              label="Principal recovered"
              value={moneyPrecise(data?.totals?.principal_component)}
              helper="Principal reduction from collected repayments."
            />
            <LoanReportMetricCard
              label="Interest recovered"
              value={moneyPrecise(data?.totals?.interest_component)}
      ...
      <Card className="overflow-hidden p-0">
        <div className="border-b border-slate-200 bg-slate-50/80 px-5 py-4">
          <CardTitle>Collections register</CardTitle>
          <p className="mt-1 text-sm text-slate-500">
            Detailed repayment entries with principal, interest, and penalty components.
          </p>
        </div>
        <div className="p-5">
          <DataTable
            ...
            emptyMessage="No repayment records matched the selected filters."
            columns={[
              {
                header: 'Borrower',
                accessor: (row) => (
              ...
              {
                header: 'Recorded',
                accessor: (row) => formatDate(row.recorded_at),
              },
              {
                header: 'Reference',
                accessor: (row) => (
              ...
              {
                header: 'Principal',
                accessor: (row) => moneyPrecise(row.principal_component),
                align: 'right',
              },
              {
                header: 'Interest',
                accessor: (row) => moneyPrecise(row.interest_component),
                align: 'right',
              },
            ]}
            renderMobileCard={(row) => (
              <article className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 shadow-sm">
                ...
                <div className="mt-4 grid gap-2 text-sm text-slate-600 sm:grid-cols-2">
                  <p>Recorded: {formatDate(row.recorded_at)}</p>
                  <p>Reference: {row.reference || '-'}</p>
                  <p>Method: {row.payment_method || 'No method'}</p>
                  <p>Amount: {moneyPrecise(row.amount)}</p>
                  <p>Principal: {moneyPrecise(row.principal_component)}</p>
                  <p>Interest: {moneyPrecise(row.interest_component)}</p>
                </div>
              </article>
            )}
          />
